...
| `docs/backtest.md` | **1년 백테스트 결과 — 기본 설정은 청산률 80~90%.** 라이브 얘기 전에 읽어라 |
...
**백테스트 결론(2026-08-20, `docs/backtest.md`)**: 지난 1년 1분봉으로 돌리면 **기본 설정은
시작일 24개 중 83~92%가 강제청산된다**(1k/296 auto는 생존 중앙값 2일, 3k/2000 auto는 23일).
2026-08-19 야간 급등(60분 +5,120 USDT)에서도 1k/296은 auto 20/20·both 19/20 청산됐다.
원인은 운이 아니라 40배에서 기준가~청산가 완충이 2~4%뿐인 구조다. **`SL_ENABLED=true`는
이 레버리지에서 무효다** — SL 가격(평단 ±3%)이 청산가보다 바깥에 있어 트리거 전에 청산된다.
시드를 늘려도 방어가 안 된다(청산 거리는 시드와 무관). 사용자에게 성과를 말할 땐 이 사실을 빼지 마라.
