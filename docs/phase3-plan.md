...
## 2026-08-18 — `1k` 프리셋 추가 (소액 시드 296 USDT 대응)

**동기**: 사용자 보유 시드가 296 USDT라 기존 프리셋으로는 아예 시작할 수 없었다
(3k 최소 약 548 / 5k 약 2,656). "1k로 296 테더가 되는지 검증하고 되면 만들어라"는 요청.

**검증 (라이브 시세/계약스펙으로 실측, 주문은 안 냄)**
BTC 64,241 / `min_qty=0.001` / `min_notional=10` 기준:

| 프리셋 | equity | 실사용 단계 | 최소주문 미달 | 최심가~청산가 완충 |
|---|---|---|---|---|
| 1k (20단계) | 296 | 17/20 | **0개** | 2.15%p |
| 1k (20단계) | 148 (both 한쪽) | 17/20 | **0개** | 2.14%p |
| 2k (40단계) | 296 | 34/40 | 0개 | 1.99%p |
| 3k (60단계) | 296 | 50/60 | 10개 (거부) | — |

- **1k 최소 시드 ≈ 65 USDT**(이분탐색), both면 130. 296은 여유 있게 통과한다.
- 완충 2.15%p로 3k(2.08)/5k(2.01)와 같은 수준 → 레버리지 40배를 그대로 유지했다.
- qty_step(0.001) 내림 손실: equity 296이면 계획 수량 합의 94.1%가 실제로 나간다
  (148이면 89.1%, 최소치 65면 76.1%). 시드가 작을수록 설계 대비 축소가 커진다.

**구현**
- `config/presets.py`: `"1k"` = 범위 1,000 USDT → 20단계(1 tier), 40배. `GridPresetName`에 추가.
- `main.py` `_warn_if_mandatory_sl_unreachable()`: `sl_enabled`인데
  `mandatory_sl_min_tier > max_stage`면 경고. 1k는 tier가 1개뿐이라 기본 `.env`
  (`MANDATORY_SL_MIN_TIER=3`)와 조합하면 필수 SL이 **영원히 등록되지 않는다** —
  조용히 넘어가면 "SL이 걸려 있다"고 오해하게 된다(2026-08-04에 같은 유형의 사고 있었음).
- `launcher.py` 즉시 진입 범위 메뉴에 "1,000 USDT (소액 시드용)" 추가(기본값은 3,000 유지).
- `scripts/paper_run.py --preset`에 `1k` 허용.
- `.env.example` / `README.md`(프리셋별 최소 자금 표) 갱신.

**부수적으로 잡은 버그 — `scripts/paper_run.py`의 프리셋 오버라이드가 먹지 않았다**
`Settings().model_copy(update=overrides)`는 **검증을 다시 돌리지 않아서**
`_apply_grid_preset`(max_stage/leverage 덮어쓰기)이 실행되지 않았다. 즉 `--preset 1k`를
줘도 `.env`의 `MAX_STAGE=3`이 그대로 남아 60단계 격자가 깔렸다(그래서 첫 1k 시도가
"296으로는 594 USDT 필요"라며 거부됨). 지금까지 `--preset 3k`가 맞아 보였던 건 로컬
`.env`가 이미 `GRID_PRESET=3k`였기 때문이지 인자가 먹은 게 아니었다. `Settings(**overrides)`로
바꿔 검증을 다시 타게 했다(안 넘긴 필드는 pydantic-settings가 `.env`에서 읽음).
`tests/test_presets.py`에 이 함정을 박제한 회귀 테스트 추가.

**paper 실행 확인(실제 시세)**: `--direction auto --preset 1k --equity 296`
→ max_stage=1/leverage=40 적용, 17단계 격자(64,206~65,006), 1단계 체결 0.004 BTC,
TP 자동 등록, 수량 샘플 `['0.004','0.005','0.005','0.006','0.006']`(전부 0.001 배수).

**테스트 271개 전부 통과.**

## 2026-08-20 — 백테스트 결과 문서화(`docs/backtest.md`) + 백테스트 하네스 커밋

**동기**: 2026-08-18에 돌린 1년 백테스트 결과가 세션 스크래치패드에만 있고 리포지터리에
아무 기록이 없었다(`docs/backtest.md` 부재 = SPEC Phase 5 미이행). 사용자 요청으로
(1) 결과를 문서화하고 (2) "어제(2026-08-19) 밤 +5,000불 급등에도 살아남았을지"를 새로 시뮬레이션했다.

**하네스를 `scripts/backtest/`로 승격**(전부 조회/계산만 하고 주문은 절대 안 낸다)
- `backtest.py`(시뮬레이터, 하드코딩 sys.path를 `parents[2]`로 교체) / `rolling.py` / `sizing.py` /
  `run5k.py`: 스크래치패드에서 그대로 이동.
- `fetch_data.py`: `--start/--end/--out` 인자화(원래 1년 고정이라 짧은 구간을 못 받았다).
- **신규** `run_year.py`(프리셋·시드 인자화한 1년 러너), `run_window.py`(하루~며칠 구간 +
  시작 시각 민감도), `run_window_variants.py`(SL/사이징/레버리지 방어 수단 비교),
  `move_stats.py`(역방향 X%가 H시간 안에 나올 확률).
- 데이터(30MB)와 산출 CSV/TXT는 `.gitignore`에 추가 — 스크립트로 언제든 재생성 가능.

**새로 나온 결과 2건**
1. **1k 프리셋 / 296 USDT(사용자 실제 시드) 1년 백테스트** — 기존 기록은 3k/5k뿐이었다.
   auto는 **0.8일**(2025-08-19)에 청산, both는 92.8일(2025-11-19)에 청산. 시작일 24개 기준
   auto 92% / both 88% 청산, **생존 중앙값 auto 2일 / both 16일**로 3k(23일/28일)보다 훨씬 나쁘다.
   원인: 1k는 절대 금액(±1,000 USDT) 프리셋이라 BTC 116,000에서는 격자 전체가 0.86%뿐이고
   (64,000에서는 1.56%) 40배 청산 거리(2~4%)보다 좁다 — 격자를 다 채운 뒤엔 단방향 40배 포지션과 같다.
2. **2026-08-19 야간 급등 구간(KST 08-19 23:35 → 08-20 00:34, 65,330 → 70,450, 60분 +5,120)**:
   1k/296 기준 **auto 20/20(100%)·both 19/20(95%) 청산**(KST 08-20 00:14~00:27).
   봉 내부 경로 3종 모두 동일. 죽기 전까지는 익절 4회로 +9.97 벌었고 청산 1회가 −271.41을 가져갔다.
   방어 수단 재실험: 사이징을 잔고의 1/4(74)로 줄이거나 레버리지 10배면 생존(각각 −40.7% / −46.5%),
   그 이상 줄이면 `min_trade_amount`에 걸려 기동 불가.

**부수적으로 확인한 것 — `SL_ENABLED=true`는 40배에서 구조적으로 무효다**
17단계 전량 체결 상태의 SL 가격 66,810.9(기준가 +3.75%) vs 같은 상태 청산가 66,716.1(+3.60%) —
**SL이 청산가보다 95 USDT 바깥**이라 트리거 전에 청산된다. 1년 백테스트에서 SL을 켠 쪽이
오히려 먼저 죽은 것(3k auto 2025-10-02 vs 10-03, 5k auto+SL 2025-12-03 vs 2026-01-31)과 일치한다.
`sl_pct=0.03`은 엑셀 원본값이고 레버리지 20배 가정에서 나온 것이라 40배와는 맞지 않는다.

**주의로 남길 것**: 유지증거금률 0.5%는 엑셀 원본에서 온 값이고 **OrangeX의 실제 리스크 리밋
티어는 라이브 확인 전**이다. 실제 값이 더 크면 청산은 이 결과보다 더 일찍 온다. 슬리피지 0 /
지정가 100% 체결 가정이라 결과는 낙관 편향이다.

**SPEC Phase 5 잔여**: 2년+ 급락 구간(2021-05/2022-06/2024-08) 백테스트, paper 30일 무중단.

## 2026-08-20 (2) — 5k 최소 시드 재검증 + 시뮬레이터 2건 보강

**요청**: "5k 기준으로 최소 시드 만족할 때 상황에서 다시 시뮬레이션".

**신규 `scripts/backtest/min_seed.py`** — 기동 가능한 최소 시드를 선형 스캔으로 실측하고
단조성까지 확인한다(이분탐색은 단조성을 가정하므로). 최소 시드는 고정값이 아니라 **시세 종속**이다:
BTC 116,184 → 5,054 / 69,408 → 2,998 / 64,683 → 2,827 (both는 2배).

**결과 요약** (자세한 표는 `docs/backtest.md` §2.5)
- 2026-08-19 야간 급등: 5k auto @2,827은 `standard`/`highfirst`에서 −61.5%로 생존,
  `lowfirst`에서는 청산(−94.6%). **청산가 70,200 vs 실제 고가 70,450 — 문턱에 걸쳐 있었다.**
  생존 케이스의 종료 상태는 0.941 BTC 숏 @67,560, 평가손 −61%, 청산까지 1.14%.
  both @5,654는 −26.7% 생존, both @2,827은 기동 불가(한쪽당 1,413 < 2,827).
- 1년: **auto @2,827은 무청산 +58.7%**지만 첫 기동이 **2026-02-05**다(171일 대기).
  즉시 기동 가능한 5,054/5,654는 2026-01-31에 −94.5%/−90.2%로 청산, 시작일 24개 중 12개(50%).
  시작일을 2025-08~2025-12 어디로 옮겨도 @2,827 결과가 전부 동일(+58.7%)한 것이 근거 —
  **최소 시드가 의도치 않은 저가 구간 진입 필터로 작동한 것**이고 전략이 안전해진 게 아니다.
- 왜 1k보다 멀리 버티나: 완충(최심→청산)은 1k 2.33%p / 5k 2.26%p로 거의 같고, 차이는 격자 폭이다.
  격자가 넓으면 평단이 위로 밀려 기준가~청산가가 3.60% → 9.01%로 벌어진다(평가손도 같이 커짐).

**시뮬레이터 보강 2건 (`backtest.py`)**
1. **체결 직후 청산 재판정 추가** — 기존에는 틱 시작 시점의 포지션 크기로만 청산을 검사해서,
   급등 1분봉에서 같은 틱에 격자가 채워지면 커진 포지션 기준 판정을 놓칠 수 있었다.
   `run_auto`/`run_both` 양쪽에 체결 후 재검사를 넣었다. **기존 1k/3k/5k 결과는 바뀌지 않았다**
   (1k @296 어제밤 구간 재실행해 소수점까지 동일 확인).
2. `min_seed.py`가 아주 작은 equity에서 `compute_grid()`의 0 나눗셈을 만나는 것을 확인 —
   스캔에서는 "기동 불가"로 집계한다.

**부수적으로 발견한 실제 엔진 결함(미해결)**: **증거금 부족으로 격자 진입 주문이 거부될 때
처리가 없다.** `_refresh_grid_orders()`의 `place_limit_order()` 예외가 `fill_router`(예외 처리 없음)를
타고 올라가 봇이 멈춘다 — 포지션은 열린 채 TP 지정가만 거래소에 남는다. 5k를 최소 시드로
돌리면 이 상황이 **예외가 아니라 정상 경로**가 된다(어제밤 구간에서 4회 발생). Phase 4 항목으로 남긴다.
...
## 2026-08-21 — 5k both "최소 시드" 백테스트 (docs/backtest.md §2.6)

사용자 요청: "5k both를 최소 시드를 만족한 버전으로 1년 백테스트, 수익률과 청산 여부".

- §2.5의 both 행(5,054 / 5,654)은 **both 최소 시드에 미달**이었다 — both는 한쪽당 절반이라
  1년 시작가 116,184에서는 10,108이 필요하다. 그 두 값은 2026-02까지 기동 자체를 못 해서
  고가 구간을 쉰 것이고, 그래서 "+130% 무청산"이 나왔던 것이다.
- 첫날부터 기동 가능한 **10,108로 돌리면 47일 만에 −94.6% 청산**(2025-10-05 @125,137).
  봉 내부 경로 3종이 같은 날 같은 가격에서 죽는다. 시작일 24개 중 20개(83%) 청산.
- 시드 18개 스윕 결과 생존 구간은 **11,800~13,000뿐**이고 그 위아래는 전부 같은 날 청산된다.
  원인은 `qty_step` 내림·최소 주문금액이 격자 수량을 불연속으로 바꾸는 것 — 즉 §2.3의
  `both @12,000 무청산`은 전략의 성질이 아니라 반올림 우연이었다. 문서에 그렇게 명시했다.

추가한 하네스: `scripts/backtest/run_both_year.py`(both 전용 1년 드라이버),
`scripts/backtest/sweep_both_seed.py`(시드 민감도), `scripts/backtest/probe_both_open.py`
(OPEN_AT_END 상세). 기존 `run_year.py`는 auto 4종을 같이 돌려 both만 볼 때 낭비가 커서 분리했다.
